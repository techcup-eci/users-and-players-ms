...
@WebMvcTest(controllers = {UserApiController.class, GlobalExceptionHandler.class})
...
class UserControllerTest {
    ...
    @Nested @DisplayName("GET /api/users/{id} - Find user by ID")
    class FindUserByIdTests {

        @Test @DisplayName("Must return 200 with user data when user exists")
        ...
        @Test @DisplayName("Must return 404 when user does not exist")
        ...
        @Test @DisplayName("Must never expose the password field in the response")
        ...
        @Test @DisplayName("Must return 400 when ID in URL is not a valid number")
        void mustReturn400WhenIdIsNotAValidNumber() throws Exception {
            mockMvc.perform(get("/api/users/abc")).andExpect(status().isBadRequest());
        }
    }

    @Nested @DisplayName("PUT /api/users/{id} - Update user")
    class UpdateUserTests {

        @Test @DisplayName("Must return 200 when update request contains valid data")
        void mustReturn200WhenUpdateRequestContainsValidData() throws Exception {
            baseUser.setFullName("Carlos Alberto Perez");
            when(userService.updateUser(eq(1L), any(), any(), any(), any())).thenReturn(baseUser);
        ...
        @Test @DisplayName("Must return 400 when full name is empty")
        ...
        @Test @DisplayName("Must return 400 when full name is null")
        ...
        @Test @DisplayName("Must return 404 when user to update does not exist")
        void mustReturn404WhenUserToUpdateDoesNotExist() throws Exception {
            ...
            mockMvc.perform(put("/api/users/99").contentType(MediaType.APPLICATION_JSON)
                    .content(objectMapper.writeValueAsString(validRequest)))
                .andExpect(status().isNotFound());
        }

        @Test @DisplayName("Must return 400 when request body includes email field which is not updatable")
        ...
        @Test @DisplayName("Must return 400 when request body is empty")
    ...
    @Nested @DisplayName("PATCH /api/users/{id}/deactivate - Deactivate user")
    class DeactivateUserTests {

        @Test @DisplayName("Must return 200 when user is successfully deactivated")
        ...
        @Test @DisplayName("Must return 409 when user is linked to an active or in-progress tournament")
        void mustReturn409WhenUserIsLinkedToActiveTournament() throws Exception {
            ...
            mockMvc.perform(patch("/api/users/1/deactivate")).andExpect(status().isConflict());
        }

        @Test @DisplayName("Must return 404 when user to deactivate does not exist")
        void mustReturn404WhenUserToDeactivateDoesNotExist() throws Exception {
            when(userService.deactivateUser(99L)).thenThrow(new UserNotFoundException("User not found"));
            mockMvc.perform(patch("/api/users/99/deactivate")).andExpect(status().isNotFound());
        }

        @Test @DisplayName("Must return 400 when ID in URL is not numeric")
        void mustReturn400WhenIdIsNotNumeric() throws Exception {
            mockMvc.perform(patch("/api/users/xyz/deactivate")).andExpect(status().isBadRequest());
        }
    }
}
